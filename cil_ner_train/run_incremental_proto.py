...
def teacher_evaluate(args, train_dataloader, model, tokenizer, labels, pad_token_label_id, mode, data_dir, prefix=""):
    ...
    # Relabeling Old Entity Classes eith prototype relabeling threshold
    preds, emissions, out_label_ids, prototype_dists = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="rehearsal",
                             data_dir=data_dir)
    ...
    for i in range(current_task_id):
        if args.change_th:
            task_para = th_para - (current_task_id - i - 1)*th_reduction
        else:
            task_para = th_para
