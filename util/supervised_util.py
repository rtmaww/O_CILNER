...
def read_example_from_file_onto(data_dir, mode):
    file_path = os.path.join(data_dir, "{}.txt".format(mode))
    guid_index = 1
    examples = []
    with open(file_path, encoding="utf-8") as f:
...
def get_labels_incr(path, order_list, per_types, nb_tasks, step_id):
    types_list = get_labels(path)
    if "O" in types_list:
        types_list.remove("O")
    labels = []
    order = order_list[:(step_id + 1) * per_types]
    for type_id in order:
        labels.append(types_list[type_id])
    if "O" not in labels:
        labels = ["O"] + labels
    return labels
...
def get_labels_dy(path, per_types, step_id):
    ...
    if "O" in types_list:
        types_list.remove("O")
    labels = types_list[:(step_id + 1) * per_types]
    if "O" not in labels:
        labels = ["O"] + labels
    print(len(labels), labels)
    return labels
def get_labels_current(path, per_types, step_id):
    types_list = get_labels(path)
    if "O" in types_list:
        types_list.remove("O")
    labels = types_list[step_id * per_types:(step_id + 1) * per_types]
    if "O" not in labels:
        labels = ["O"] + labels
    print(len(labels), labels)
    return labels